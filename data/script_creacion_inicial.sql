...
CREATE TABLE [NUEVE_Z].[TIPO_DESCUENTO_VENTA] (
  [TIPO_DESCUENTO_VENTA_ID] [DECIMAL](19,0) IDENTITY(1,1) NOT NULL,
  [DESCUENTO_VENTA_CONCEPTO] [NVARCHAR](255) NULL,
  PRIMARY KEY ([TIPO_DESCUENTO_VENTA_ID]),
)ON [PRIMARY]
...
CREATE TABLE [NUEVE_Z].[DESCUENTO_VENTA] (
  ...
  [VENTA_ID] [DECIMAL](19,0) NULL,
  [TIPO_DESCUENTO_VENTA_ID] [DECIMAL](19,0),
  PRIMARY KEY ([DESCUENTO_VENTA_CODIGO]),
  CONSTRAINT [FK_DESCUENTO_VENTA.VENTA_ID]
    FOREIGN KEY ([VENTA_ID])
        REFERENCES [NUEVE_Z].[VENTA](VENTA_ID),
  CONSTRAINT [FK_DESCUENTO_VENTA.TIPO_DESCUENTO_VENTA_ID]
    FOREIGN KEY ([TIPO_DESCUENTO_VENTA_ID])
        REFERENCES [NUEVE_Z].[TIPO_DESCUENTO_VENTA](TIPO_DESCUENTO_VENTA_ID)
)ON [PRIMARY]
...
/*VARIANTE*/
/* No se puede sacar el JOIN de VP? */
...
FROM [gd_esquema].[Maestra] M 
JOIN [NUEVE_Z].[PROVINCIA] P ON (M.CLIENTE_LOCALIDAD = P.PROVINCIA_NOMBRE)
...
FROM [gd_esquema].[Maestra] M 
JOIN [NUEVE_Z].[PROVINCIA] P ON (M.PROVEEDOR_PROVINCIA = P.PROVINCIA_NOMBRE)
WHERE M.PROVEEDOR_LOCALIDAD IS NOT NULL AND M.PROVEEDOR_LOCALIDAD NOT IN (SELECT LOCALIDAD FROM NUEVE_Z.LOCALIDAD)

INSERT INTO [NUEVE_Z].[LOCALIDAD] (LOCALIDAD, PROVINCIA_CODIGO)
SELECT DISTINCT M.PROVEEDOR_LOCALIDAD, P.PROVINCIA_CODIGO
FROM [gd_esquema].[Maestra] M 
JOIN [NUEVE_Z].[PROVINCIA] P ON (M.CLIENTE_PROVINCIA = P.PROVINCIA_NOMBRE)
WHERE M.CLIENTE_LOCALIDAD IS NOT NULL AND M.CLIENTE_LOCALIDAD NOT IN (SELECT LOCALIDAD FROM [NUEVE_Z].[LOCALIDAD])
/*CLIENTES Y PROVEEDORES TIENEN UN PAR DE LOCALIDADES DIFERENTES*/
...
/*
Tambien me parece mas prolijo hacer 1 solo insert
INSERT INTO [NUEVE_Z].[LOCALIDAD_CODIGO_POSTAL] (CP_CODIGO, LOCALIDAD_CODIGO)
SELECT DISTINCT M.CLIENTE_CODIGO_POSTAL, L.LOCALIDAD_CODIGO 
FROM [gd_esquema].[Maestra] M
...
SELECT DISTINCT M.PROVEEDOR_CODIGO_POSTAL, L.LOCALIDAD_CODIGO 
FROM [gd_esquema].[Maestra] M
...
FROM [gd_esquema].[Maestra] M
JOIN [NUEVE_Z].[LOCALIDAD] L ON (M.PROVEEDOR_LOCALIDAD = L.LOCALIDAD)
WHERE M.PROVEEDOR_CODIGO_POSTAL IS NOT NULL AND L.LOCALIDAD_CODIGO IS NOT NULL AND M.PROVEEDOR_CODIGO_POSTAL NOT IN (SELECT CP_CODIGO FROM [NUEVE_Z].[LOCALIDAD_CODIGO_POSTAL])
...
SELECT DISTINCT M.VENTA_CODIGO, M.VENTA_FECHA, VC.VENTA_CANAL_CODIGO, VC.VENTA_CANAL_COSTO, VE.ENVIO_CODIGO, VM.MEDIO_ENVIO_PRECIO, VP.MEDIO_DE_PAGO_VENTA_CODIGO, VP.MEDIO_PAGO_COSTO, C.CLIENTE_CODIGO
FROM [gd_esquema].[Maestra] M
...
JOIN [NUEVE_Z].[CODIGO_POSTAL] CP ON(CP.CODIGO = M.CLIENTE_CODIGO_POSTAL)
JOIN [NUEVE_Z].[LOCALIDAD] L ON(M.CLIENTE_LOCALIDAD = L.LOCALIDAD)
JOIN [NUEVE_Z].[MEDIO_DE_ENVIO] VM ON (M.VENTA_MEDIO_ENVIO = VM.MEDIO_DE_ENVIO AND M.VENTA_ENVIO_PRECIO = VM.MEDIO_ENVIO_PRECIO AND CP.CODIGO = VM.CP_CODIGO AND L.LOCALIDAD_CODIGO = VM.LOCALIDAD_CODIGO)
...
JOIN [NUEVE_Z].[CLIENTE] C ON (M.CLIENTE_DNI = C.CLIENTE_DNI)
JOIN [NUEVE_Z].[ENVIO] VE ON (VE.MEDIO_ENVIO_CODIGO = VM.MEDIO_ENVIO_CODIGO)
...
/*TIPO_DESCUENTO_VENTA*/
INSERT INTO [NUEVE_Z].[TIPO_DESCUENTO_VENTA] (DESCUENTO_VENTA_CONCEPTO)
SELECT DISTINCT VENTA_DESCUENTO_CONCEPTO FROM [gd_esquema].[Maestra] M
WHERE VENTA_DESCUENTO_CONCEPTO IS NOT NULL

/*DESCUENTO_VENTA*/
INSERT INTO [NUEVE_Z].[DESCUENTO_VENTA] (DESCUENTO_VENTA_IMPORTE, VENTA_ID,TIPO_DESCUENTO_VENTA_ID)
SELECT DISTINCT M.VENTA_DESCUENTO_IMPORTE, V.VENTA_ID, TD.TIPO_DESCUENTO_VENTA_ID
FROM [gd_esquema].[Maestra] M JOIN [NUEVE_Z].[VENTA] V
ON (M.VENTA_CODIGO = V.VENTA_CODIGO)
JOIN [NUEVE_Z].[TIPO_DESCUENTO_VENTA] TD 
ON (M.VENTA_DESCUENTO_CONCEPTO = TD.DESCUENTO_VENTA_CONCEPTO)

/*VENTA -> VALOR_TOTAL*/
UPDATE [NUEVE_Z].[VENTA] 
SET VENTA_TOTAL = 
        (SELECT DISTINCT SUM(DV.VARIANTE_PRODUCTO_CANTIDAD * DV.PRECIO_UNITARIO_VENTA) +
				IsNull((SELECT V.VENTA_ENVIO_PRECIO FROM [NUEVE_Z].[VENTA] V WHERE V.VENTA_ID = DV.VENTA_ID),0) - 
				IsNull((SELECT SUM(CPV.VALOR_DESCUENTO) FROM [NUEVE_Z].[CUPON_POR_VENTA] CPV WHERE CPV.VENTA_ID = DV.VENTA_ID),0) -
				IsNull((SELECT SUM(DEV.DESCUENTO_VENTA_IMPORTE) FROM [NUEVE_Z].[DESCUENTO_VENTA] DEV WHERE DEV.VENTA_ID = DV.VENTA_ID),0) 
					FROM [NUEVE_Z].[DETALLE_VENTA] DV 
					WHERE DV.VENTA_ID = [NUEVE_Z].[VENTA].VENTA_ID
