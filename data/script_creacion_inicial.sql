...
SET VENTA_TOTAL = (SELECT DISTINCT SUM(DV.VARIANTE_PRODUCTO_CANTIDAD * DV.PRECIO_UNITARIO_VENTA) +  [NUEVE_Z].[VENTA].VENTA_ENVIO_PRECIO - SUM(CPV.VALOR_DESCUENTO) - SUM(DEV.DESCUENTO_VENTA_IMPORTE)
					FROM [NUEVE_Z].[DETALLE_VENTA] DV 
					JOIN [NUEVE_Z].[CUPON_POR_VENTA] CPV
					ON (CPV.VENTA_ID = DV.VENTA_ID)
					JOIN [NUEVE_Z].[DESCUENTO_VENTA] DEV
					ON (DEV.VENTA_ID = DV.VENTA_ID)
					WHERE [NUEVE_Z].[VENTA].VENTA_ID = DV.VENTA_ID)
