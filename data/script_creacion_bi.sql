...
--------DIM_RANGO_ETARIO----------
INSERT INTO [NUEVE_Z].[BI_DIMENSION_RANGO_ETARIO] (LIMITE_INFERIOR, LIMITE_SUPERIOR, RANGO_ETARIO)
VALUES (0, 24,'<25')
INSERT INTO [NUEVE_Z].[BI_DIMENSION_RANGO_ETARIO] (LIMITE_INFERIOR, LIMITE_SUPERIOR, RANGO_ETARIO)
VALUES (25, 34, '25-35')
INSERT INTO [NUEVE_Z].[BI_DIMENSION_RANGO_ETARIO] (LIMITE_INFERIOR, LIMITE_SUPERIOR, RANGO_ETARIO)
...
((SUM(DV.PRECIO_UNITARIO_VENTA * DV.VARIANTE_PRODUCTO_CANTIDAD) - SUM(DC.PRECIO_UNITARIO_COMPRA * DC.VARIANTE_PRODUCTO_CANTIDAD))/(SUM(DV.PRECIO_UNITARIO_VENTA * DV.VARIANTE_PRODUCTO_CANTIDAD))),
SUM(DV.VARIANTE_PRODUCTO_CANTIDAD),
SUM(DC.VARIANTE_PRODUCTO_CANTIDAD),
...
JOIN [NUEVE_Z].[VENTA] V ON (V.VENTA_ID = DV.VENTA_ID)
JOIN [NUEVE_Z].[BI_DIMENSION_TIEMPO] T ON (YEAR(V.VENTA_FECHA) = T.ANIO AND MONTH(V.VENTA_FECHA) = T.MES)
JOIN [NUEVE_Z].[CLIENTE] CLI ON (CLI.CLIENTE_CODIGO = V.CLIENTE_CODIGO)
JOIN [NUEVE_Z].[BI_DIMENSION_RANGO_ETARIO] RE ON ((DATEDIFF(YEAR, CLI.CLIENTE_FECHA_NAC, CAST(GETDATE() AS DATE))) >= RE.LIMITE_INFERIOR AND (DATEDIFF(YEAR, CLI.CLIENTE_FECHA_NAC, CAST(GETDATE() AS DATE))) <= RE.LIMITE_SUPERIOR)
JOIN [NUEVE_Z].[COMPRA] COMP ON (COMP.COMPRA_ID = DC.COMPRA_ID)
JOIN #PRECIOPROMPROV PPP ON (PPP.PROVEEDOR_CODIGO = COMP.PROVEEDOR_CODIGO AND YEAR(COMP.COMPRA_FECHA) = PPP.COMPRA_FECHA_ANIO)
...
--------1----------
GO
CREATE VIEW [NUEVE_Z].[GANANCIAS_CANAL_VENTA] (ANIO, MES, ID_CANAL_DE_VENTA, TIPO_CANAL_VENTA, GANANCIAS)
AS
...
AS
    SELECT DISTINCT T.ANIO, T.MES, RE.RANGO_ETARIO,OTRATEMP.CATEGORIA, OTRATEMP.VENTA_PRODUCTO AS TOTAL_VENDIDO
    ...
    JOIN (SELECT SUM(HP2.VENTA_PRODUCTO) AS VENTA_PRODUCTO, RE2.RANGO_ETARIO, T2.ANIO, T2.MES, CP2.CATEGORIA, rank()
                    over (PARTITION BY RE2.RANGO_ETARIO, T2.ANIO, T2.MES
                            order by SUM(HP2.VENTA_PRODUCTO) DESC) as rank
				...
				JOIN [NUEVE_Z].[BI_DIMENSION_CATEGORIA_PRODUCTO] CP2 ON (CP2.ID_DIM_CATEGORIA_PRODUCTO = HP2.ID_DIM_CATEGORIA_PRODUCTO)
				GROUP BY RE2.RANGO_ETARIO, T2.ANIO, T2.MES, CP2.CATEGORIA
